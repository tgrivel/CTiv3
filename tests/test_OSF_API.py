...
class TestOSFAPI:
    class TestBase(unittest.TestCase):
        ...
        @unittest.skip("Test overgeslagen omdat OSF deze functionaliteit nog niet in de API aanbiedt.")
        def test_bestand_met_meerdere_fouten_geeft_http500_en_fouten(self):
            fouten_element_referentie = \
        ...
        def test_controle_geef_totalen_uitkomst_voor_bestand_met_PF(self):
            fouten_element_referentie = ["Fout: een plausibiliteitsfout."]
...
class TestGeefFoutenURL(TestOSFAPI.TestBase):
    def setUp(self):
        self.osf_url = OSF_API_GEEF_FOUTEN_URL


class TestGeefTotalenUitkomstURL(TestOSFAPI.TestBase):
    def setUp(self):
